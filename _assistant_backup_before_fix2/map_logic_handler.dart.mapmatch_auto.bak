...
class MapLogicHandler {
    ...
    if (_displayPos == null) {
      ...
      try {
        if (_userSymbol == null && controller != null) {
          controller!.addSymbol(SymbolOptions(geometry: _displayPos!, iconImage: 'assets/icons/profile_icon.png', iconSize: 0.40)).then((sym) {
            _userSymbol = sym;
          }).catchError((e) {});
  ...
  Future<void> snapTo(LatLng pos, {bool moveCamera = false}) async {
    ...
    try {
      if (_userSymbol == null && controller != null) {
        _userSymbol = await controller!.addSymbol(SymbolOptions(geometry: pos, iconImage: 'assets/icons/profile_icon.png'));
      } else if (_userSymbol != null && controller != null) {
