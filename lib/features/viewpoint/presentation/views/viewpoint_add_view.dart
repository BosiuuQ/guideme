...
class _ViewpointAddViewState extends State<ViewpointAddView> {
  // Map/location picker variables (copied from SpotAddView)
  ...
  Future<void> _pickImage() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Nie można wybrać zdjęcia: \$e')));
      return;
    }
  ...
  Future<void> _cropCurrentImage() async {
    ...
    File? cropped;
    try {
      cropped = await ImageCropperHelper.crop(file: _imageFile!);
    } catch (e) {
      debugPrint('Image crop failed: \$e');
                      ...
                      child: Form(
                        ...
                        child: Column(
                          children: [
                            ...
                            // Label for minimap (replaces previous input + button above the map)
                            Align(
                            ...
                            // Row with "Użyj mojej pozycji" (left) and selected coords (right, expanded)
                            Row(
                              children: [
                                ElevatedButton(
                                  onPressed: _useMyLocation,
                                  child: const Text('Użyj mojej pozycji'),
                                ),
                                const SizedBox(width: 12),
                                Expanded(
                                  child: _pickedLocation != null
                                      ? Text(
                                    'Wybrano: ${_pickedLocation!.latitude.toStringAsFixed(5)}, ${_pickedLocation!.longitude.toStringAsFixed(5)}',
  ...
  // Use my location helper (copied behaviour)
