...
class MapboxMapController {
  ...
  Future<Symbol> addSymbol(SymbolOptions options) async {
    ...
    final sym = Symbol(id, options.geometry, iconImage: options.iconImage, rotation: options.rotation);
    _symbols[id] = sym;
    return sym;
  }
  ...
  Future<void> updateSymbolImmediate(Symbol symbol, LatLng newPos) async {
    ...
    try {
      ...
      for (final s in _symbols.entries) {
        final sym = s.value;
        final pos = sym.geometry;
        Uint8List? imageBytes;
        try {
          if (sym.iconImage != null) {
            final bd = await rootBundle.load(sym.iconImage!);
            imageBytes = bd.buffer.asUint8List();
          }
        } catch (_) {}
        final nativeOptions = native.PointAnnotationOptions(
